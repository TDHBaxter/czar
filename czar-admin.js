...
discordBot.login('NTE3NTM2NzAwNjMzODQxNjc3.DuDq0A.58qxsIRk6Z2PPPlHOvm5_-7Wn24');
const discordGuild = config.discord_guild_id;

var discordChannelName = function(label) {
    var discord_channel = label.toLowerCase();
    discord_channel = discord_channel.replace(/[^0-9a-z]*[._][^0-9a-z]*/g, "_");
    discord_channel = discord_channel.replace(/[^0-9a-z_][^0-9a-z_]*/g, "-");
    discord_channel = discord_channel.replace(/^[_-]*/g, "");
    discord_channel = discord_channel.substring(0, 40);
    discord_channel = discord_channel.replace(/[_-]*$/g, "");
    ...
    if (input.className == "dirty") {
      ...
      if (input.name == "label") {
        if (!input.value && input.czar_oldvalue) {
          ...
          channelName = discordChannelName(input.value);
          discordBot.guilds.get(discordGuild).channels.forEach((value,key,map) => {
            if (value.name == channelName) {
              value.setParent(config.discord_deleted);
            }
          });  
        } else if (input.value) {
        ...
        if (input.name == "tags") {
          UpdateTagsSelector();
          MaybeHandleSolvedPuzzle(this.label,this.name, input.value);
        }
      }
      if (!input.className)
...
var gSolvedPuzzles = [];
var MaybeHandleSolvedPuzzle = function(puz_label, puzzle_id, new_tags) {
  czarDebug('MaybeHandleSolvedPuzzle(' + puzzle_id + ', ' + new_tags + ')');
  channelName = discordChannelName(puz_label.value);
  ...
  if (tags.indexOf('solved') == -1) {
    ...
    if (gSolvedPuzzles.indexOf(puzzle_id) != -1) {
      ...
      discordBot.guilds.get(discordGuild).channels.forEach((value,key,map) => {
        if (value.name == channelName) {
          value.setParent(config.discord_unsolved);
        }
      });  
    }
    return;
  ...
  var iteratorSolveCats = config.discord_solved.values();
  solvecat = "";
  for (const value of iteratorSolveCats) {
    if (discordBot.guilds.get(discordGuild).channels.get(value).children.size < 49) {
      solvecat = value;
    }
  }
  discordBot.guilds.get(discordGuild).channels.forEach((value,key,map) => {
    if (value.name == channelName) {
      value.setParent(solvecat);
    }
  });  
...
var on_submit_create = function() {
  ...
  if (this.label.className == "dirty") {
    ...
    var discord_channel = discordChannelName(label);
    ...
    if (label) {
      ...
      urlpromise = gGoogleAccess.createSheet(label, function(id, url) {
      ...
      discordChannel = discordBot.guilds.get(discordGuild).createChannel(discord_channel).then(newChannel => newChannel.setParent(config.discord_unsolved)).catch(console.error);
      discordChannel.then(newChannel => {send_value(name, "discord", "discord://discordapp.com/channels/"+discordGuild+"/"+newChannel.id); return(newChannel);}).catch(console.error);
      Promise.all([discordChannel,urlpromise]).then(function(valArray) {
        console.log(valArray);
        valArray[0].setTopic(valArray[1]); }).catch(console.error);
...
var the_form_html =
  ...
  "</span>" +
  "<span class=tootltip_wrap>" +
    "<a class=missing target=@NAME@.discord id=@NAME@.discord>discord</a>" +
    "<span class=tooltip><input type=text name=discord size=30></span>" +
  "</span>" +
...
var make_form = function(name) {
  ...
  bind_link(form, "discord", "Click to enter Discord channel URL");
